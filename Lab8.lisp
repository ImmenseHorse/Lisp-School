    ...
    (if (car res)
        (if (atom e)
            ...
            (case (car e)
              ...
              ((let) (dolist (kvl (cadr e))
                       (unless (and (assoc (nth 0 kvl) env) (update! (nth 0 kvl) env (evaluate (nth 1 kvl) env)))
                           (setf env (append (list (cons (nth 0 kvl) (evaluate (nth 1 kvl) env))) env))))
               (evaluate (append '(progn) (cddr e)) env))
              ((if) (if (evaluate (cadr e) env) (evaluate (caddr e) env) (evaluate (cadddr e) env)))
